...
要解决这样的问题，就必须要对数据进行同步控制。Java中，利用了对象的“互斥锁”机制来实现线程间的互斥操作。即一个线程在对一个对象中的资源进行访问时，它拿到了这个对象的“锁“，那其它线程就不可以再访问这个对象中被保护起来的资源，必须等有”锁“的线程执行完毕，释放掉”锁“。

Java语言中，使用`synchronized`关键字来标识共享的资源，以实现数据的同步控制。这个关键字的功能就是：首先判断对象的互斥锁是否在，若在线程就获得互斥锁，然后就可以访问共享资源，如果不在，线程就进入等待，直到获得锁才能对共享资源进行访问。
...
程序运行结果符合预期，可以说线程是安全的。这是因为我们用`synchronized`关键字修饰了take()方法，对其进行了同步限制，所以在一个线程，比如'c1'线程执行完take()方法之前，'c2'线程不能进入take()方法，执行代码对共享数据'sum'进行访问，从而避免了一个线程对共享数据的修改结果覆盖了另一线程对共享数据的修改结果。那么，共享的数据'sum'就应该要设置为`private`权限，不然的话，通过**对象名.属性名**就能访问到'sum'，这样对共享数据的保护就没有意义了。
...
我们可以这样理解：'c1'线程获得了'bank'对象的锁之后，由于'bank'对象是静态的，属于'Mbank'类，所以是唯一的，那对象锁只有一个，'c2'线程只能等待锁被释放掉，才能去访问'bank'对象中被`synchronized`标记或者说保护起来的资源。

那我们把'bank'对象的`static`去掉呢？
...
程序运行结果又不符合预期了，这是因为去掉了`static`之后，每创建一个线程对象，在线程对象内部又会各自实例化自己的'bank'对象，这样每个线程都拿到了'bank'对象的锁，但不是同一个'bank'对象的锁，而是自己的'bank'对象的锁。

所以，要理解'互斥锁'的含义，它锁的是一个实例化对象，一个对象只有一个互斥锁，利用这种对锁的争夺，就可以实现不同线程的互斥效果了。

除了对象锁之外，还有另一种类型的锁，即`类锁`。这一般用于两个地方：
